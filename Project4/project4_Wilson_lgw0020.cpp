/**
 * @file project4_Wilson_lgw0020.cpp
 * @author Laura Wilson
 * @brief Project 4 trivia quiz
 * @version 0.1
 * @date 2022-04-06
 * 
 * @copyright Copyright (c) 2022
 * 
 * complie: g++ project4_Wilson_lgw0020.cpp
 * run: ./a.out project4_Wilsonlgw0020.cpp
 * outsider sources: The hint sheet 
 * 
 */
...
int main() {
   ...
   string cont_input;
   cout << "*** Welcome to Laura's trivia quiz game ***\n";
...
void init_question_list(ptr_node& q_list) {
   ...
   cur_ptr->ques = "How long was the shortest war on record? (Hint: how many minutes)";
   cur_ptr->ans = "38";
   cur_ptr->point = 100;
   cur_ptr->next = cur_ptr2;
   num_of_questions++;
	//Create second question node and declare its values
   cur_ptr2->ques = "What was Bank of America's original name? (Hint: Bank of Italy or Bank of Germany)?";
   cur_ptr2->ans = "Bank of Italy";
   cur_ptr2->point = 50;
   cur_ptr2->next = cur_ptr3;
   num_of_questions++;
	//Create third question node and declare its values
   cur_ptr3->ques = "What is the best-selling video game of all time? (Hint Call of Duty or Wii Sports)?";
   cur_ptr3->ans = "Wii Sports";
   cur_ptr3->point = 20;
   cur_ptr3->next = NULL;
   num_of_questions++;
}
...
void add_question(ptr_node& q_list) {
   ...
   getline(cin, new_q->ques);

	//User is asked for an answer
   cout << "Enter an answer: ";
   getline(cin, new_q->ans);
...
int ask_question(ptr_node q_list, int num_ask) {
   ...
   //Loop through the linked list, get user's answers, detemines correctness, and calculates point value
      ...
      for (int x = 0; x < num_ask; x++) {
         cout << "Question: " << cur_ptr->ques << endl;
         cout << "Answer: ";
         getline(cin, user_ans);
      	//If correct the points are incremented
         if (user_ans.compare(cur_ptr->ans) == 0) {
         ...
         //If incorrect the points remain the same
            ...
            cout << "Your answer is wrong. The correct answer is " << cur_ptr->ans << endl;
            cout << "Your total points: " << point_total << endl;
         }
